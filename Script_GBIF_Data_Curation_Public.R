...
#Change infraspecific ranks like ssp to subsp. 
#Remove author names to enable taxon query
...
#I will also extract the accepted name for each syonomyn and also add it, to make sure I get all possible names. 
{
...
#The species name without the author information cannot be matched, for this will result in mutliple matches due to the author name being the only distinguishing factor among some species names. 
...
#An example could be "Gnaphalium norvegicum Retz.", which when searched using "Gnaphalium norvegicum"
...
#and therefore the same epithet as the specific epithet (e.g. Saxifraga paniculata subsp. paniculata), can be manually checked by a taxonomic expert and re-added into the confirmed master list. This is because of 
#Article 26 of The International Code of Nomenclature for algae, fungi, and plants. Specifically where it specifies that, 
#"The name of any infraspecific taxon that includes the type of the adopted, legitimate name of the species to which it is assigned is to repeat the specific epithet unaltered as its 
#final epithet, not followed by an author citation (see Art. 46). Such names are autonyms (Art. 6.8; see also Art. 7.7). Ex. 1. The variety that includes the type of the name Lobelia spicata Lam. is to be named Lobelia spicata Lam. var. spicata 
#The first instance of valid publication of a name of an infraspecific taxon under a legitimate species name automatically establishes the corresponding autonym (see also Art. 11.6 and 32.3). Ex. 5. The publication of the name Lycopodium inundatum var. bigelovii Tuck. (in Amer. J. Sci. Arts 45: 47. 1843) 
#automatically established the name of another variety, L. inundatum L. var. inundatum, the autonym, the type of which is that of the name L. inundatum L. (Art. 7.7).
...
cat(paste("Autonyms$species_name[Autonyms$species_name == '", df_Autonyms3$original_input_name, "'] <- 'Write_Corrected_Name_Here'\n", sep = ""), sep = "\n") #Changes the Autonyms$species_name accordingly 

#Example of what to paste in here below (without hash tag):
...
#Now re-run the code with spellings corrected and cheked by an expert taxonomist 
...
#For example, I have a case of an autonym like _Argentina egedii_ subsp. _egedii_, and to correct it to an 
#accepted name, its parent species name is _Argentina egedii_ (Wormsk. ex Oeder) Rydb. Á.Löve - so then the question 
#becomes, what is _Argentina egedii_ subsp. _egedii_t to the autonym of that accepted name. But now I have a case where 
#_Argentina egedii_ is actually a synonym of _Argentina anserina_ subsp. _groenlandica_ (Tratt.) Á.Löve - so then the question becomes, what is _Argentina egedii_ subsp. _egedii_? 
  
#In this case, the autonym’s standing is overridden by the synonymy of its parent species. 
#Since the species Argentina egedii is a synonym, its infraspecific taxa, including the 
#autonym Argentina egedii subsp. egedii, would also be treated as a synonym. Therefore 
#given that autonyms are the typical infraspecific rank of that parent species, I would 
#list Argentina egedii subsp. egedii as a synonym of Argentina anserina subsp. groenlandica as 
#well, rather than an independent autonym. 
...
#Problems typically occur in 6 main categories 
...
{
  #PASTE HERE THE CODE THAT WAS PRINTED FOR YOU, AND EDIT AS NEEDED. HERE ARE SOME EXAMPLES:
  
  #(1) Names with minor differences in spelling to the species name, which could be corrected:
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Triglochin palustre L.'] <- "Triglochin palustris L." #Same spelling on herbarium sheet. Confirmed (same genus and authority, likely altnerative species spelling) on Page 99 in Book "Dansk flora 2. udgave (2012) Frederiksen, Rasmussen, and Seberg"
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Ligusticum scoticum L.'] <- "Ligusticum scothicum L." #Same spelling on herbarium sheet. Confirmed alternative spelling in "Palin, M. A. (1988). Ligusticum Scoticum L. (Haloscias Scoticum (L.) Fr.). Journal of Ecology, 76(3), 889–902. https://doi.org/10.2307/2260580"
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Saxifraga hieracifolia Porter'] <- "Saxifraga hieraciifolia Porter" #Herbarium Sheet is actually listed as  "Saxifraga hieraciifolia Porter"

  #(2) Names with the author spelled differently than is officially standardized by POWO. They are edited below. Note that in some herbarium sheets, the authority names are spelled differently or are not even included.  
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Papaver lapponicum subsp. occidentale (Lundstr.) Knaben'] <- "Papaver lapponicum subsp. occidentale (Markgr.) Knaben" 
  ...
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Eriophorum triste (T.C.E.Fr.) Hadac & Á.Löve'] <- "Eriophorum triste (Th.Fr.) Hadac & Á.Löve" #Authority name not spelled properly according to KEWR. Species likely found in Removed_Master_List                                           
  
  # (3) Names where the specimens' labels differed than the GBIF interpreted scientific name, and this specimen label name is recognized by POWO, which could be corrected.
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Festuca vivipara var. vivipara'] <- "Festuca vivipara subsp. vivipara" #Herbarium Sheet is actually listed as  "Festuca vivipara ssp. vivipara". Synonym - Sourced from Integrated Taxonomic Information System - https://www.itis.gov/servlet/SingleRpt/SingleRpt;jsessionid=A10D0E36493DCA05B6240A63936C5149?search_topic=TSN&search_value=40799#null
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Cystopteris fragilis var. fragilis'] <- "Cystopteris fragilis subsp. fragilis"    #Herbarium Sheet is actually listed as  "Cystopteris fragilis (L.) Bernh. ssp. Fragilis"
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Taraxacum spectabilia'] <- "Taraxacum sect. spectabilia" #Herbarium Sheet is actually listed as "Taraxacum sect. spectabilia" - Confirmed Section status by HAGLUND, G. E. (1983). New species of Taraxacum, section Spectabilia, from Swedish Lapland. Annales Botanici Fennici, 20(3), 245–257. http://www.jstor.org/stable/23725884
  
  #(4) Names which were not recognized by POWO, but which may be specific to the location of query and thus need to be checked in another more localized flora source
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Festuca brachyphylla var. supina'] <- 'Unknown' #Same spelling as herbarium sheet
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Erigeron unalaschkensis var. subanedo*'] <- 'Unknown' #Same spelling as herbarium sheet
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Draba alpina f. pulvinata'] <- 'Unknown' #Same spelling as herbarium sheet
  
  # (5)	Genera rank or higher rank names which were not recognized by POWO, but which may be specific to the location of query and thus need to be checked in another more localized flora source. 
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Agropyrum'] <- "Unknown"                                          
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Athyriaceae'] <- "Unknown"                                       
  #Leftovers$Manually_Edited_Name[Leftovers$name == 'Hippuridaceae'] <- "Unknown"
  
  # (6) Names which were incorrectly transcribed, but which are still not recognized by POWO after being corrected 
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Potentilla arenosa subsp. dramissonis'] <- 'Potentilla arenosa subsp. chamissonis × niveae'#Herbarium sheet actually labelled as Potentilla arenosa subsp. chamissonis x nivea but not in powo 
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Dryas integrifolia x octopetala Vahl x L.'] <- 'Dryas integrifolia x octopetala' #Re-labelled herbarium sheet 
  #Leftovers$Manually_Edited_Name[Leftovers$original_input_name == 'Carex gynocrates Wormsk. x lachenalii Schkuhr'] <- 'Carex gynocrates x lachenalii'
       }
{
...
###########################################################
### END of Generating Taxon List and Leftovers List #####
###########################################################
#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#~~~~~~#
...
#Before counting the number of GBIF specimens found for each accepted name, it is important to first compare the taxon list
#against an additional external flora, local to the region(s) under investigation. This will allow for potentially missed names
...
cat("During this step, the Taxon List and the Leftover Names will be compared to another database, ideally a local flora\n")
...
Reference_List<- #Upload here the reference list, fitting the criteria listed above
...
#For example, the reference list used for Greenland lists Antennaria intermedia (Rosenv.) Porsild as an accepted name. However in POWO, 
...
incorrect_rows <- as.numeric(strsplit(readline(prompt = "Enter the row numbers of incorrect matches (comma-separated): "), ",")[[1]]) #Enter here row numbers for Greenland -1,2,3,4,6,7,9,11,12,13,16,18,19,22
...
#An example for Greenland is Dryopteris austriaca Schinz & Thell., which according to the reference flora is a synonym of Dryopteris expansa (C.Presl) Fraser-Jenk. & Jermy, 
...
#To run an internal validation, we can use the number of occurrences for a given species
...
#Autonyms - given that autonyms are auto-generated due to:
#Article 26 of The International Code of Nomenclature for algae, fungi, and plants. Specifically where it specifies that, 
#"The name of any infraspecific taxon that includes the type of the adopted, legitimate name of the species to which it is assigned is to repeat the specific epithet unaltered as its 
#final epithet, not followed by an author citation (see Art. 46). Such names are autonyms (Art. 6.8; see also Art. 7.7). Ex. 1. The variety that includes the type of the name Lobelia spicata Lam. is to be named Lobelia spicata Lam. var. spicata 
#The first instance of valid publication of a name of an infraspecific taxon under a legitimate species name automatically establishes the corresponding autonym (see also Art. 11.6 and 32.3). Ex. 5. The publication of the name Lycopodium inundatum var. bigelovii Tuck. (in Amer. J. Sci. Arts 45: 47. 1843) 
...
#These names are considered to be reliable, and therefore should not be flagged. 
#However, this can still be manually determined:
...
Specimen_Counts_Final_NoAutonyms <- Specimen_Counts_Final_NoAutonyms %>%
  ...
  select(Accepted_Name_with_Author, Flagged_Name, Flagged_Specimen_Count, Flagged_DataSource_Count)
...
#1. They should be removed, as in they are an error, like Arthropteris tenella (G.Forst.) J.Sm. for Greenland
#2. If they should be unflagged, as we are confident it belongs, like Lupinus nootkatensis Donn ex Sims for Greenland
...
#Next, make use of KEW POWO database to unflag specimens known to occur in the country you are using
#The KEW POWO database is missing many names which are IN the country you are using, but we are confident 
#that the ones known to be in the country you are using in KEW POO are correct, just not exhaustive. 
...
Basionyms<- do.call(rbind.data.frame, List1$basionymOf) 
...
#Now an Excel is created to check these cases:
...
cat("Please open Exclusive_Taxa_List_Truly_Unmatched.xlsx and do the following:\n\n",
    "In the column Confirmed, type REMOVE if the taxa name should be removed upon checking it, or YES if it should remain. Ones validated by the Reference Flora are already pre-confirmed.\n\n",
    "In the Comment column, note down any important notes leading to the decision.")

write_xlsx(Confirmed_Master_List_Tidy_ExclusiveTaxaList_SpeciesAndBelow_Truly_Unmatched, "Exclusive_Taxa_List_Truly_Unmatched.xlsx")
...
Confirmed_Master_List_Tidy_ExclusiveTaxaList_SpeciesAndBelow_Truly_Unmatched <- read_excel("Exclusive_Taxa_List_Truly_Unmatched.xlsx")

{
#Remove REMOVE species from the Confirmed_Master_List_Tidy.
Removed_Curation_Publish<-Confirmed_Master_List_Tidy_ExclusiveTaxaList_SpeciesAndBelow_Truly_Unmatched%>%
  filter(Confirmed=="REMOVE") %>%
  rename(Verified_Name = Accepted_Name_with_Author) %>%
  mutate(original_input_name=NA,
         status="Manually_Removed") %>%
  select(-species,-Confirmed)
...
{
  New_Species_In_Region<-Confirmed_Master_List_Tidy_ExclusiveTaxaList_SpeciesAndBelow_Truly_Unmatched%>%
  select(Accepted_Name_with_Author,Confirmed,rank)
  
Confirmed_Master_List_Tidy_New_Species_In_Region<-Confirmed_Master_List_Tidy%>%
  filter(Accepted_Name_with_Author %in% New_Species_In_Region$Accepted_Name_with_Author)

#When possible, extract parent species information:
Filtered_Classification <- Confirmed_Master_List_Tidy_New_Species_In_Region %>%
  filter(
...
New_Species_In_Region <- New_Species_In_Region %>%
...
New_Species_In_Region<-New_Species_In_Region%>%distinct()%>%
  filter(!rank=="Race")
...
#Hierchloe alpina is a synonym! Accent issue in Autonyms df? 
write_xlsx(New_Species_In_Region, "New_Species.xlsx")
...
#Ref_Names_Multiple_POWO_Publish
...
Final_Removed_Names<-rbind(Leftover_autonyms_Publish,Ref_Names_Multiple_POWO_Publish,Unauthored_Names_Publish,Final_Leftovers_Publish,Removed_Flags_Publish,Removed_Curation_Publish,Unplaced_Removed_Publish)
